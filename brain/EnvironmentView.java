...
public class EnvironmentView extends JFrame
{
    ...
    private static final Color[] colors = {
        ...
        //Color(0*i/256, 191*i/256, 255*i/256)  //celeste, i ∈ (0, 256) para intensidad
        //Color(240*i/256 128*i/256, 128*i/256) //rosado i ∈ (0, 256) para intensidad
    };
    ...
    public void showCells()
    {
        ...
        for(int row = 0; row < cells.length; row++) {
            ...
            for(int col = 0; col < numCols; col++) {
                ...
                //write if statements to distiguish between black and white notes
                if (cellRow[col].getTypeOfNote() == 1) {
                    view.drawMark(col, row, new Color(240*state/256,128*state/256, 128*state/256)); //pink
                }
                if (cellRow[col].getTypeOfNote()  == 0) {
                    view.drawMark(col, row, new Color(0*state/256, 191*state/256, 255*state/256)); //pink
                }
            }
        }
    ...
    {
        //23 milisegundos
        delay = 23; //((100.0 - speedPercentage) * LONGEST_DELAY / 100);
    }
    ...
    private class GridView extends JPanel
    {
        ...
        {
            g.setColor(color);
            g.fillRect(x * xScale, y * yScale, xScale, yScale); //substract small 'n' from last to params to create spaced-grid
            // for example
            //g.fillRect(x * xScale, y * yScale, xScale-1, yScale-1); , which is the standard 
        }
